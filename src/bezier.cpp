...
void GLRenderer::apply_bezier_matrices(glm::vec3 P0, glm::vec3 P1, glm::vec3 P2, glm::vec3 P3){
    for (int i=0; i<m_grass_matrices.size(); i++){
        float t = *m_grass_z_buffer[i]/-m_radius;
        ...
        glm::vec4 prior_pos = *m_original_grass_matrices[i]*glm::vec4(0,0,0,1);
        m_grass_matrices[i] =
            std::make_shared<glm::mat4>
            (*m_original_grass_matrices[i]*glm::translate(glm::vec3(0,curve.y-prior_pos.y,curve.z-prior_pos.z))*glm::rotate(-theta,glm::vec3(1,0,0)));
    }
...
//    P3 = P3 + glm::vec3(0,0.75,0.75);
    for (int i=0; i<m_building_matrices.size(); i++){
        ...
        m_building_matrices[i] =
            std::make_shared<glm::mat4>
            (*m_original_building_matrices[i]*glm::translate(glm::vec3(0,curve.y-prior_pos.y,curve.z-prior_pos.z))*glm::rotate(-theta,glm::vec3(1,0,0)));
    }
